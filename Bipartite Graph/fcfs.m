...
for user_id = 1:n
    ...
    frame_deadline = [(users{selected_users(user_id)}.frame_deadline(1:end)) ; frame_deadline ];
end
...
user = {};
user = struct('number_of_packets_per_frame', number_of_packets_per_frame, 'frame_release_times', frame_release_times, 'frameSizeB', frameSizeB, 'fps', fps, 'nf', nf, 'value', value, 'frame_deadline', frame_deadline);

%frame_release_times = frame_release_times*ones((user.nf) .* (user.number_of_packets_per_frame),1);
