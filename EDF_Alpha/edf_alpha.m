...
while(temp_packet_index < total_number_of_packets)
    ...
    if(~isempty(available_packet_queue))
        [~, max_packet_index] = max(available_packet_queue);
        [~, edf_index] = min(available_packet_queue_deadlines);
        if available_packet_queue(edf_index) >= available_packet_queue(max_packet_index)/alpha       % Weight_e >= Weight_MaxW/alpha
