function [scheduled_order, waiting_time, system_time] = FCFS(Virtual_Queue, num_users, time_slots)
%t1 = 0;
%t2 = 0;
...
if size(scheduled_order,2) > 1
    ...
    scheduled_order = [scheduled_order(temp_order_indices), frame_order(temp_order_indices), user_order(temp_order_indices), QoE_order(temp_order_indices)];
end  
...
%i = 1;
%while (i <= 60000) 
   if scheduled_order(i,5) < scheduled_order(i,1)
